...
export default function initializeDocument(args) {
    return {
        ...
        markText: function (event) {
            ...
            if (selectedText !== '') {
                ...
                while (
                    startOffset > 0 &&
                    !'s|-|,|/|\\'.test(
                        range.startContainer.textContent[startOffset - 1],
                    )
                ) {
                    startOffset--
                }
                ...
                while (
                    endOffset < range.startContainer.textContent.length &&
                    !'s|-|,|/|\\'.test(
                        range.startContainer.textContent[endOffset],
                    )
                ) {
                    endOffset++
                }
                ...
                if (!isWithinLabeledSpan) {
                    ...
                    if (!overlappingLabel) {
                        ...
                        this.labelList.set(key, {
                            ...
                            color: this.genColorCodeFromText(
                                this.selectedLabel.name,
                            ),
                        })
                        ...
                        newNode.style.backgroundColor =
                            this.genColorCodeFromText(this.selectedLabel.name)
        ...
        refreshTextField: function () {
            ...
            const sortedLabelList = [...this.labelList.entries()].sort(
                (a, b) => a[1].start - b[1].start,
            )
            sortedLabelList.forEach(([key, label]) => {
                if (currentIndex < label.start) {
                    resultHTML += this.targetText.substring(
                        currentIndex,
                        label.start,
                    )
                }
        ...
        removeLabel: function (event) {
            if (event.target.classList.contains('dynamic-span')) {
                event.preventDefault()
                const key = Array.from(event.target.classList).find((cls) =>
                    this.labelList.has(cls),
                )
                if (key) {
                    this.labelList.delete(key)
                    this.refreshTextField()
